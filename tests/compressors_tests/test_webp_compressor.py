...
class TestWebPRunCwebp:
    ...
    def test_z_flag_for_level(self, compressor, level, expected_z):
        with patch("compressors.webp_compressor.run_with_affinity", return_value=_ok()) as mock_run:
    ...
    def test_raises_runtime_error_on_nonzero_return_code(self, compressor):
        with patch("compressors.webp_compressor.run_with_affinity", return_value=_fail(1, "cwebp failed")):
...
class TestWebPCompress:
    ...
    def test_success(self, compressor, tmp_path):
        ...
        with patch("compressors.webp_compressor.run_with_affinity", side_effect=fake_run):
    ...
    def test_failure_on_cwebp_error(self, compressor, tmp_path):
        ...
        with patch("compressors.webp_compressor.run_with_affinity", return_value=_fail(1, "cwebp error")):
...
class TestWebPDecompress:
    ...
    def test_dwebp_binary_is_invoked(self, compressor, tmp_path):
        ...
        with patch("compressors.webp_compressor.run_with_affinity", side_effect=fake_run) as mock_run:
    ...
    def test_raises_on_dwebp_error(self, compressor, tmp_path):
        ...
        with patch("compressors.webp_compressor.run_with_affinity", return_value=_fail(1, "dwebp failed")):
