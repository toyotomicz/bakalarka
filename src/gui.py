...
class BenchmarkGUI:
    ...
    def _log_welcome(self) -> None:
        ...
        self.log("Image Compression Benchmark")
    ...
    def open_upload_dialog(self) -> None:
        ...
        if self._upload_dialog is None or not self._upload_dialog.winfo_exists():
            ...
            # Dialog already open, bring it to the front and focus it
    ...
    def open_results_folder(self) -> None:
        """
        Open the benchmark results folder in Windows File Explorer.
        """
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                f"Could not open results folder:\n{exc}"
            )
    ...
    def _benchmark_thread(self, config: BenchmarkConfig) -> None:
        ...
        """
        def ui_call(fn, *args) -> None:
            self.root.after(0, fn, *args)

        try:
            results, verification_results = self.runner.run(
                progress_callback=lambda msg: ui_call(self.log, msg)
            )

            if results:
                ui_call(self._save_and_summarize, results, verification_results, config)
            else:
            ...
            import traceback
            ui_call(self.log, f"\nError: {exc}")
            ui_call(self.log, traceback.format_exc())

        finally:
            ui_call(self._on_benchmark_finished)
    ...
    def _save_and_summarize(self, results, verification_results, config: BenchmarkConfig) -> None:
        ...
        # Auto open the visualization window with the last written JSON file
    ...
    def open_visualization(self) -> None:
        """Prompt the user to select a JSON file and open the visualization window."""
        json_dir = self.output_dir / "json_reports"
    ...
    def _open_visualization_with_data(self, data: BenchmarkData, auto_show: bool = False) -> None:
        """
        Open the visualization window for the given benchmark data.

        Args:
            data: Preloaded BenchmarkData object.
...
def main() -> None:
    ...
    try:
        ...
        pass  # fallback to default theme if "clam" is unavailable for some reason
    BenchmarkGUI(root)
    root.mainloop()
